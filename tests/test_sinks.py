...
def test_sqlite_upsert_on_iso2(tmp_path):
    ...
    rows1 = [{"country":"France","capital":"Paris","iso2":"FR","continent":"Europe"}]
    rows2 = [{"country":"France","capital":"PARIS","iso2":"FR","continent":"Europe"}]  # changed capital

    from sinks import SqliteSink
    with SqliteSink(str(db), "countries",
                    columns=["country","capital","iso2","continent"],
                    upsert_keys=["iso2"], upsert_update="all") as s:
        s.write(rows1)
        s.write(rows2)
